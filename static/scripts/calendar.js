...
function getDatesBetween(date1,date2) {
    ...
    for (let i=0; i<dates.length; i++){
        ...
        content += 
        "<h2 id='" + firstDate.toString().split(" ")[1] + "'>" + 
...
let eventWindow = document.getElementById('event-info');
let dateInput = document.getElementById('event-date');
theCalendar.addEventListener("click", chooseDate);

function chooseDate(e) {
    // show the new event window
    eventWindow.classList.remove("hide");

    
    // include the selected date
    let chosenDate = e;
    dateInput.value = firstDate.toString().split(" ")[1] + "-" + chosenDate.target.innerHTML + "-" + firstDate.getFullYear() ;
    console.log("'" + firstDate.toString().split(" ")[1] + "'");
    console.log(chosenDate.target.innerHTML);
}

function submitEvent () {
    eventWindow.classList.add("hide");
}

// exit "new event" window
